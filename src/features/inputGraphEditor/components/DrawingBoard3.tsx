...
const DrawingBoard3: React.FC = () => {
  ...
  const handleFloorplanClick = (floorplanIndex: number) => {
    console.log("Floorplan clicked:", floorplanIndex);
    // Call your desired function here
    // e.g., handleSelectedFloorplan(listRFP[floorplanIndex]);
  };
  const loadRFP = () => {
      ...
      fabric.Image.fromURL(
        ...
        (img) => {
          ...
          group.on("mousedown", () => {
            ...
            const padding = 10;
            const rect = new fabric.Rect({
              left: group.left! - 60 - padding,
              top: group.top! - padding,
              width: group.width! * group.scaleX! + padding * 2,
              height: group.height! * group.scaleY! + padding * 2,
              fill: "transparent",
              stroke: "black",
              strokeWidth: 3,
              selectable: false,
  ...
  useEffect(() => {
    if (currentIndex != -1) {
      ...
      // fabric.Image from an URL string called on click
      fabric.Image.fromURL(
        svgUrl2,
        (img) => {
          // Set position of the SVG image to top-left corner
